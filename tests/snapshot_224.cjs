...
for (const name of ['Divzer','Sunstar','Warp']) {
    assert.ok(items.some(i=>i.name===name),'original weapon remains');
    assert.ok(!items.some(i=>i.name==='Masterwork '+name),'snapshot ascension removed');
}
assert.equal(context.majorData.VORTEX,undefined);
