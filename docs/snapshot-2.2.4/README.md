...
This is an experimental snapshot fork, not a claim of complete game parity. The live and `beta-api.wynncraft.com/v3/ability/tree/shaman` endpoints still returned the previous Ritualist tree when checked on September 8. The user requested provisional tree connections and omission of new ascensions.
...
- New ascensions are omitted as requested. Other new items/ingredients, Dreamer's Soul's unnamed new major ID, unpublished lore, the Ornate Shadow set correction and unquantified fixes await authoritative data.
